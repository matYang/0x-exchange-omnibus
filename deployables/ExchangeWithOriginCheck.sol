    ...
    IAssetProxyDispatcher
{
    ...
    // Flag on if origin check should be enabled
    bool public _isOriginCheckEnabled = true;

    // If origin check is enabled, only address in this mapping is allowed to be the tx sender
    mapping(address => bool) public _allowOriginMapping;

    // either skips due to _isOriginCheckEnabled not turned on, or forces msg.sender to be one of allowed origins
    // this works in 0x Transactions as delegateCall() does not change msg.sender
    modifier onlyAllowedOrigin() {
        require(!_isOriginCheckEnabled || _allowOriginMapping[msg.sender], "Origin not allowed");
        _;
    }

    /// @dev enables origin check, only allowed origins can fill/cancel orders
    function enableOriginCheck() external onlyOwner {
        _isOriginCheckEnabled = true;
    }

    /// @dev disables origin check, any address can fill/cancel orders
    function disableOriginCheck() external onlyOwner {
        _isOriginCheckEnabled = false;
    }

    /// @dev Add an address to allowed origins
    /// @param from Address to be added
    function addOrigin(address from) external onlyOwner {
        _allowOriginMapping[from] = true;
    }

    /// @dev Removes an address from allowed origins
    /// @param from Address to be removed
    function removeOrigin(address from) external onlyOwner {
        _allowOriginMapping[from] = false;
    }
